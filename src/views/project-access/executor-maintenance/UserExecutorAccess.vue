        ...
        <template slot-scope="{data}">
          <tbody>
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="product-category">{{ tr.expired_date }}</p>
              </vs-td>

              <vs-td>
                 <vs-progress :percent="getPercentage(tr.total_job_list,tr.job_list_finish)" :height="2" color="success" style="display:flex">progress</vs-progress>
                 <small>{{ getPercentage(tr.total_job_list,tr.job_list_finish) }} % finish</small>
              </vs-td>

              <vs-td>
                <vs-chip :color="getStatusColor(tr.status)" class="product-order-status">{{ tr.status }}</vs-chip>
              </vs-td>
...
<script>
import AddNewDataSidebar from './AddNewDataUserExecutorAccessSidebar.vue';
...
export default {
  ...
  methods: {
    ...
    getPercentage(total,finish){
        return (finish/total*100).toFixed(2);
    },
    formatData(data) {
  ...
  created() {
    this.dataTable = this.$store.state.userExecutorAccess.project1;
  },
  mounted() {
    this.isMounted = true;
  }
